...
$sql_images = 'SELECT w.gallery_id gallery_id, w.id id, w.name name FROM (
	SELECT row_number() OVER (PARTITION BY gallery_id ORDER by name) AS row_number, id, name, gallery_id FROM image WHERE gallery_id IN ('.implode(',', $galleries_id).')
) as w
WHERE w.row_number <= 4';
